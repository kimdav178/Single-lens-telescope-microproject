...
def backward_ray1(x1, y1, x2, y2, rp, gp, bp):
    x3 = x2 - x1
    y3 = y2 - y1
    if max(x2 + (x3 - x2) * l / FDO, y2 + (y3 - y2) * l / FDO) > RadL:
        return 0, 0, 0, 0, 0
    else:
        x4 = x2 + (x3 - x2) * l / FDO
        y4 = y2 + (y3 - y2) * l / FDO
        x5 = (x4 - x2) * FDL / l
        y5 = (y4 - y2) * FDL / l
        if (abs(x4 + (x5 - x4) * d/FDL) > k2*width * k/2) or (abs(y4 + (y5 - y4) * d/FDL) > k2*height * k/2):
            return 0, 0, 0, 0, 0
        else:
            x6 = x4 + (x5 - x4) * d / FDL
            y6 = y4 + (y5 - y4) * d / FDL
            return x6, y6, rp/(k2*4*pi*((d+l+FDO)*(d+l+FDO))), gp/(k2*4*pi*((d+l+FDO)*(d+l+FDO))), bp/(k2*4*pi*((d+l+FDO)*d+l+FDO))
"""
